...
function applyOceanField(c,sign){
  const large=Number(c.wave_height_m)>=2,short=Number(c.primary_period_s)<9;
  const clean=["clean","light"].includes(String(c.wind_quality).toLowerCase());
  const palette=large?["#081a38","#35204f",sign.theme[0],"#075b67","#11152e"]:["#b8e4f2","#f1efb1","#d5f2dc",sign.theme[1],"#dcefee"];
  ["--field-a","--field-b","--field-c","--field-d","--field-base"].forEach((name,i)=>document.documentElement.style.setProperty(name,palette[i]));
  document.body.dataset.swell=large?"large":"small";document.body.dataset.periodSpeed=short?"short":"long";document.body.dataset.windShape=clean?"clean":"messy";
}
function render(){
